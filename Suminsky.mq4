...
int AbrePosicao(double price,int type){
      ...
      printf("Abre posição %f %i",price,type);
      ticket = myOrderSend(type, price, Lot, type);
      if(ticket <= 0) return -1;
      else //not autotrading => only send alert
      myAlert("order", type);
      double TP = price + (Linhas * myPoint * multiplier); //Take Profit = value in points (relative to price)
    ...
    for(int i = 0; i < total; i++)
     {
      ...
      if(OrderOpenPrice() > Value){ Value = OrderOpenPrice();}
     }
   return(Value);
  }
...
void ReopenLastClosedOrder()
  {
    ...
    for(i=0;i<hstTotal;i++)
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_HISTORY)==false) continue;
        ...
        if(OrderMagicNumber() != MagicNumber || OrderSymbol() != Symbol()) continue;
       // printf("MagicNumber ok.OT %i : Last %i",OrderTicket(),LastClosedOrder);
...
int ReopenClosedOrder(int ticket)
  {
    ...
    if(OrderSelect(ticket, SELECT_BY_TICKET)==true){
        ...
        lasttype = OrderType();
        printf("Ultima ordem %i - ultimo preço %f",ticket , lastprice );
        printf(lasttype);
      ...
      for(int i = 0; i < total; i++)
      {
        ...
        if(OrderOpenPrice() == lastprice){ 
          printf("Encontrou Ordem com mesmo preço %f" , lastprice);
          return false;
          }
